   ...
   (const TrackingRecHit *hit, const TrajectoryStateOnSurface &tsos, Id2 simtk, bool mustProject) const
{
   ...
   if (typeid(*hit) == typeid(SiStripMatchedRecHit2D)) {
      ...
      fillCluster(&mhit.originalHit(),   tsos, simtk, true);
...
void ClusterShapeDebugTrajectoryFilter::init
  (const edm::Event &event, const edm::EventSetup &es) const
{
  ...
  event.getByLabel(edm::InputTag("MeasurementTrackerEvent"), mte);
  bool reindex = (theMTEvent == 0);
  theMTEvent = mte.product();
  if (reindex) indexStripDets();
}

void ClusterShapeDebugTrajectoryFilter::done
   () const
{
