...
//TEST functions
template <typename T>
std::tuple<bool, float> ScPhase2PuppiHPhiGammaDemo::isolationQ(
    float eta, float phi, unsigned int pidex1, unsigned int pidex2, const T *cands, unsigned int size) const {
  float psum = 0;
...
template <typename T>
std::tuple<bool, float> ScPhase2PuppiHPhiGammaDemo::isolationTkEm(
...
template <typename T>
ROOT::Math::PtEtaPhiMVector ScPhase2PuppiHPhiGammaDemo::pairP4(const std::array<unsigned int, 2> &t,
                                                               const T *cands,
                                                               const std::array<float, 2> &massD) {
