...
template <typename T>
void ScPhase2RecMesonHPhiGamma::runObj(const OrbitCollection<T> &src,
                                    const OrbitCollection<T> &srcMeson,
                                    edm::Event &iEvent,
                                    unsigned long &nTry,
                                    unsigned long &nPass,
...
template <typename T>
bool ScPhase2RecMesonHPhiGamma::isolationTkEm(float pt, float eta, float phi, const T *cands, unsigned int size) const {
  bool passed = false;
  float psum = 0;
