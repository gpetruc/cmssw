...
class ScPhase2RecMesonHRhoGamma : public edm::stream::EDProducer<> {
public:
  explicit ScPhase2RecMesonHRhoGamma(const edm::ParameterSet &);
  ~ScPhase2RecMesonHRhoGamma() override;
...
ScPhase2RecMesonHRhoGamma::ScPhase2RecMesonHRhoGamma(const edm::ParameterSet &iConfig)
  ...
  if (doStruct_) {
    ...
    produces<l1ScoutingRun3::OrbitFlatTable>("recMesonHrhogamma");
  }
}

ScPhase2RecMesonHRhoGamma::~ScPhase2RecMesonHRhoGamma() {};

void ScPhase2RecMesonHRhoGamma::beginStream(edm::StreamID) {
  countStruct_ = 0;
  passStruct_ = 0;
}

void ScPhase2RecMesonHRhoGamma::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  if (doStruct_) {
...
void ScPhase2RecMesonHRhoGamma::endStream() {
  if (doStruct_)
    edm::LogImportant("ScPhase2AnalysisSummary") << "Rec Meson HRhoGamma Struct analysis: " << countStruct_ << " -> " << passStruct_;
}

template <typename T, typename U>
void ScPhase2RecMesonHRhoGamma::runObj(const OrbitCollection<T> &srcGamma,
                                    const OrbitCollection<U> &srcMeson,
                                    edm::Event &iEvent,
                                    unsigned long &nTry,
                                    unsigned long &nPass,
  ...
  auto tab = std::make_unique<l1ScoutingRun3::OrbitFlatTable>(bxOffsets, "recMesonHrhogamma" + label, true);
  ...
  iEvent.put(std::move(tab), "recMesonHrhogamma" + label);
}

template <typename T>
bool ScPhase2RecMesonHRhoGamma::isolationTkEm(float pt, float eta, float phi, const T *candsGamma, unsigned int size) const {
  bool passed = false;
  float psum = 0;
...
template <typename T, typename U>
float ScPhase2RecMesonHRhoGamma::tripletmass(const std::array<unsigned int, 2> &t,
                                              const T *candsGamma,
                                              const U *candsMeson) {
...
template <typename T, typename U>
float ScPhase2RecMesonHRhoGamma::tripletpt(const std::array<unsigned int, 2> &t,
                                              const T *candsGamma,
                                              const U *candsMeson) {
  ROOT::Math::PtEtaPhiMVector p1(candsMeson[t[0]].pt(), candsMeson[t[0]].eta(), candsMeson[t[0]].phi(), candsMeson[t[0]].mass());
  ROOT::Math::PtEtaPhiMVector p2(candsGamma[t[1]].pt(), candsGamma[t[1]].eta(), candsGamma[t[1]].phi(), 0);
...
void ScPhase2RecMesonHRhoGamma::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
...
DEFINE_FWK_MODULE(ScPhase2RecMesonHRhoGamma);