...
class ScPhase2RecMesonZ2Phi : public edm::stream::EDProducer<> {
public:
  explicit ScPhase2RecMesonZ2Phi(const edm::ParameterSet &);
  ~ScPhase2RecMesonZ2Phi() override;
...
ScPhase2RecMesonZ2Phi::ScPhase2RecMesonZ2Phi(const edm::ParameterSet &iConfig)
  ...
  if (doStruct_) {
    ...
    produces<l1ScoutingRun3::OrbitFlatTable>("recMesonZ2phi");
  }
}

ScPhase2RecMesonZ2Phi::~ScPhase2RecMesonZ2Phi() {};

void ScPhase2RecMesonZ2Phi::beginStream(edm::StreamID) {
  countStruct_ = 0;
  passStruct_ = 0;
}

void ScPhase2RecMesonZ2Phi::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
  if (doStruct_) {
...
void ScPhase2RecMesonZ2Phi::endStream() {
  if (doStruct_)
    edm::LogImportant("ScPhase2AnalysisSummary") << "Rec Meson Z2Phi Struct analysis: " << countStruct_ << " -> " << passStruct_;
}

template <typename T>
void ScPhase2RecMesonZ2Phi::runObj(const OrbitCollection<T> &src,
                                    edm::Event &iEvent,
                                    unsigned long &nTry,
                                    unsigned long &nPass,
  ...
  auto tab = std::make_unique<l1ScoutingRun3::OrbitFlatTable>(bxOffsets, "recMesonZ2phi" + label, true);
  ...
  iEvent.put(std::move(tab), "recMesonZ2phi" + label);
}

float ScPhase2RecMesonZ2Phi::pairmass(const l1Scouting::RecMeson *cands, int a, int b) {
...
void ScPhase2RecMesonZ2Phi::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
...
DEFINE_FWK_MODULE(ScPhase2RecMesonZ2Phi);
